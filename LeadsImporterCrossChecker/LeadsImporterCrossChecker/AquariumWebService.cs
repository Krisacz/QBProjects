    ...
    public class AquariumWebService
    {
        ...
        public List<string> GetAllLeads()
        {
            try
            {
                ...
                var leadIdCustomerId = new List<string>();
                var customerIdColumnName = "CustomerID";

                foreach (var reportSetting in _settings.ReportIds)
                {
                    var id = reportSetting.Id;
                    var leadIdColumnName = reportSetting.LeadIdColumnName;
                    var reportData = GetReportData(id);
                    var leadIdIndex = -1;
                    var customerIdIndex = -1;

                    for (var i = 0; i < reportData.Headers.Count; i++)
                    {
                        if (reportData.Headers[i] == leadIdColumnName) leadIdIndex = i;
                        if (reportData.Headers[i] == customerIdColumnName) customerIdIndex = i;
                    }

                    foreach (var reportDataRow in reportData.Rows)
                    {
                        var leadId = reportDataRow.Data[leadIdIndex];
                        var customerId = reportDataRow.Data[customerIdIndex];
                        list.Add(leadId);
                        leadIdCustomerId.Add($"{leadId},{customerId}");
                    }
                }

                File.WriteAllLines("_Aquarium-LeadId-CutomerId.txt", leadIdCustomerId);

                return list;
            }
            catch (Exception ex)
            {
                _logger.AddError($"AquariumWebService >>> GetAllLeads:", ex);
            }
        ...
        #region FLOW
        private ReportData GetReportData(int reportId)
        {
            try
            {
